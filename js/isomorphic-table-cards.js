...
class IsomorphicTableCards {
	...
	layout(keepStyle){
		...
		const cardsPerRow = Math.floor((window.innerWidth -40)/this.config.cardWidth);
		const fnTop = isCards ? idx => Math.floor(idx/cardsPerRow)*this.config.cardHeight+'px'
			: idx => 40+idx*this.config.rowHeight+'px'
		const fnLeft = isCards ? idx => (idx%cardsPerRow)*this.config.cardWidth+'px'
			: idx => 0
		const id2idx = {}
		...
		if(!keepStyle){
			this.holder.querySelector('.header').style = 'left:'+(this.curStyle==='cards' ? '-650px' : 0);
